...
def make_metric_heatmap_2x2(summary_df: pd.DataFrame, metric: str, output_dir: Path) -> None:
    ...
    fig.suptitle(f"{metric} summary by cluster and model", fontsize=18)
...
def make_clusterwise_geo_nse_map(merged: pd.DataFrame, basin_geo: pd.DataFrame, output_dir: Path) -> None:
    """
    Create a 7-panel geographic figure for the Cluster-wise model:
      - top row: one full-width CONUS map with all basins colored by cluster
      - lower rows: 6 cluster-specific subplots (3x2 layout) with basin points
        colored by basin-wise seed-mean NSE, using one shared continuous scale.

    This keeps one overview panel and six cluster-detail panels in a single figure.
    """
    ...
    lon_pad = 4.0
    lat_pad = 3.0
    xlim = (plot_df["lon"].min() - lon_pad, plot_df["lon"].max() + lon_pad)
    ylim = (plot_df["lat"].min() - lat_pad, plot_df["lat"].max() + lat_pad)
    ...
    gs = fig.add_gridspec(4, 2, height_ratios=[1.35, 1.0, 1.0, 1.0])

    # -------------------------------------------------
    # Top full-width panel: all basins colored by cluster
    # -------------------------------------------------
    ax_top = fig.add_subplot(gs[0, :])
    if usa is not None:
        try:
            usa.boundary.plot(ax=ax_top, color="0.55", linewidth=0.8, zorder=1)
        except Exception:
            pass
    ...
    for spine in ax_top.spines.values():
        ...
        spine.set_color("0.4")
    ax_top.set_title(
        "CONUS distribution of the six basin clusters",
        fontsize=15,
        pad=10,
    )
    ax_top.legend(handles, labels, loc="center left", bbox_to_anchor=(1.01, 0.5), frameon=True, title="Clusters")

    # -------------------------------------------------
    # Bottom 6 panels: one panel per cluster, colored by seed-mean NSE
    # -------------------------------------------------
    last_sc = None
    for idx, cl in enumerate(clusters[:6]):
        ...
        if usa is not None:
            try:
                usa.boundary.plot(ax=ax, color="0.75", linewidth=0.5, zorder=1)
            except Exception:
                pass
        ...
        med = float(sub[PRIMARY_METRIC].median()) if not sub.empty else np.nan
        mean_ = float(sub[PRIMARY_METRIC].mean()) if not sub.empty else np.nan
        ax.set_title(f"Cluster {cl} (median = {med:.3f}, mean = {mean_:.3f})", fontsize=11)
        ax.set_xlim(*xlim)
        ax.set_ylim(*ylim)
    ...
    if last_sc is not None:
        cbar = fig.colorbar(last_sc, ax=fig.axes, fraction=0.018, pad=0.015)
