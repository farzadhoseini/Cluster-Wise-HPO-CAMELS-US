...
def make_clusterwise_geo_nse_map(merged: pd.DataFrame, basin_geo: pd.DataFrame, output_dir: Path) -> None:
    """
    Create a 2x3 geographic panel figure showing basin-wise seed-mean NSE
    for the Cluster-wise model, separately for each cluster.
    """
    ...
    vmin = float(plot_df[PRIMARY_METRIC].min())
    vmax = float(plot_df[PRIMARY_METRIC].max())
    if np.isclose(vmin, vmax):
        vmax = vmin + 1e-6
    ...
    for idx, cl in enumerate(clusters):
        ...
        if usa is not None:
            try:
                usa.boundary.plot(ax=ax, color="0.65", linewidth=0.6, zorder=1)
            except Exception:
                pass
        ...
        ax.set_title(f"Cluster {cl} (median = {med:.3f})", fontsize=11)
        ax.set_xlim(*xlim)
        ax.set_ylim(*ylim)
    ...
    if last_sc is not None:
        cbar = fig.colorbar(last_sc, ax=axes.tolist(), fraction=0.025, pad=0.02)
