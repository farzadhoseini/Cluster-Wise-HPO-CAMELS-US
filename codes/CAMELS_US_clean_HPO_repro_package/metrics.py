...
def mean_absolute_percentage_error(obs: DataArray, sim: DataArray) -> float:
    r"""Calculate the mean absolute percentage error (MAPE).

    .. math:: \mathrm{MAPE} = \frac{100}{T}\sum_{t=1}^T \left|\frac{\widehat{y}_t - y_t}{y_t}\right|,

    where :math:`\widehat{y}` are the simulations (`sim`) and :math:`y` are observations (`obs`).

    Observations that are zero are excluded to avoid division by zero.
    ...
    # exclude zero‐observations to avoid division by zero
    nonzero_mask = obs_vals != 0
    obs_vals = obs_vals[nonzero_mask]
    sim_vals = sim_vals[nonzero_mask]

    # if nothing left, return nan
    if obs_vals.size == 0:
        return np.nan

    # compute MAPE
    mape = np.mean(np.abs((sim_vals - obs_vals) / obs_vals)) * 100
    return float(mape)
...
def calculate_all_metrics(obs: DataArray,
    ...
    results = {
        ...
        "MAPE": mean_absolute_percentage_error(obs, sim)
    }
...
def calculate_metrics(obs: DataArray,
    ...
    for metric in metrics:
        if metric.lower() == "nse":
            ...
            values["MAPE"] = mean_absolute_percentage_error(obs, sim)
        else:
